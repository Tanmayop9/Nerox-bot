...
export default class Avatar extends Command {
    ...
    execute = async (client, ctx) => {
        ...
        const serverAvatar = member?.avatar ? member.displayAvatarURL({ dynamic: true, size: 4096 }) : null;
        ...
        if (serverAvatar && userAvatar !== serverAvatar) {
            const row = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId('user_avatar').setLabel('User Avatar').setStyle(ButtonStyle.Primary),
                new ButtonBuilder()
