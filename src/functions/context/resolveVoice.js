export const resolveVoice = async (ctx, command) => {
    ...
    if (!memberVc) {
        await ctx.reply({
            embeds: [client.embed().desc(`${client.emoji.cross} You must be in a voice channel to use this command.`)],
        });
        return false;
    }
    if (command.inSameVC && botVc && botVc.id != memberVc.id) {
        await ctx.reply({
            embeds: [client.embed().desc(`${client.emoji.cross} You must be in ${botVc} to use this command.`)],
        });
        return false;
    }
    return true;
