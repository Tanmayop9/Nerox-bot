export const resolveBotAdmin = async (ctx, command) => {
    ...
    if (command.mod) {
        if (!mod) {
            await ctx.reply({
                embeds: [client.embed().desc(`${client.emoji.cross} Mods only. You ain't got the rank for this.`)],
            });
            return false;
        }
        return true;
    ...
    if (command.serverstaff) {
        if (!serverStaff) {
            await ctx.reply({
                embeds: [client.embed().desc(`${client.emoji.cross} Only VIP servers can use this. Yours ain't one.`)],
            });
            return false;
        }
        return true;
