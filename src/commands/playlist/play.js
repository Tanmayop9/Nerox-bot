...
export default class PlaylistPlay extends Command {
    constructor() {
        super(...arguments);
        this.name = 'pl-play';
        this.aliases = ['plplay', 'playlist-play'];
        this.usage = '<name>';
        this.inSameVC = true;
        this.description = 'Play all tracks from a playlist';
        this.options = [
            {
                name: 'name',
                required: true,
        ...
        this.execute = async (client, ctx, args) => {
            ...
            const msg = await ctx.reply({
                embeds: [client.embed().desc(`Loading **${playlist.name}**...`)],
            });
            ...
            await msg.edit({
                embeds: [
                    client.embed().desc(
                        `Now playing **${playlist.name}** with **${added}** tracks. ` +
                        `${added < playlist.tracks.length ? `(${playlist.tracks.length - added} tracks couldn't be loaded)` : ''}`
                    )
                ],
            });
        };
